...
float flex_data_mean[] = {0, 0, 0, 0, 0};
float flex_data_median[] = {0, 0, 0, 0, 0};
...
void __gen_data_string() {
  sprintf(buffer, "%f,%f,%f", flex_data[1], flex_data_mean[1], flex_data_median[1]);
...
void __read_flex_data() {
  for(int i = 0; i < 5; i++) {
    flex_data[i] = analogRead(FLEX_PINS[i]);
  }
}
